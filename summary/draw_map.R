...
register_google(key = "AIzaSyBUwCpLbYxM1tPd0yAeQLQo-5cH3DSLSF0")
sofia_map<-get_googlemap(center = c(23.320, 42.660), zoom = 12, key = "AIzaSyBUwCpLbYxM1tPd0yAeQLQo-5cH3DSLSF0")
bounds<-as.numeric(attr(sofia_map, "bb"))

# read data
particles<-read.csv("clean_data/2018-10_sds011sof_result.csv")
...
ggsave("image/pm2.5_1810.png", width = 7, height = 7)
...
p2_map + geom_point(aes(color = sqrt(P2), size = P2, alpha = P2)) + 
         ...
         scale_alpha_continuous(range = c(0.5, 1)) + 
         guides(size = FALSE, alpha = FALSE)
ggsave("image/pm10_1810.png", width = 7, height = 7)
